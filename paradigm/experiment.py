...
if not dlg.OK or info['ID'] == '': core.quit();

# Stimulus list and randomization (see randomization.py)
...
# Timing
if debugging:
    FIX = 0; CUE = 0; STIM = 60; BLANK = 0; FB = .25; ITI = 0; BLOCKFB = 5
else:
    FIX = .5; CUE = 1; STIM = 5; BLANK = .5; FB = .5; ITI = 1; BLOCKFB = 120

# Initialize stimuli
win = visual.Window(fullscr = True, units="norm", gammaErrorPolicy='ignore')
...
debug_instr = visual.TextStim(win = win, text = 'Press "backspace" to skip a block, press "esc" to quit.',\
                              pos = [0,-.7], height = .05, wrapWidth = 1.75, font = 'monospace')
...
for block in np.arange(1,7): # block loop
    for phase_i, phase in enumerate(['prac_cued', 'prac_free', 'cued', 'free']): # phase loop
        
        # no practice after first block
        if block > 1:
            if phase == 'prac_cued' or phase == 'prac_free': continue
        
        # set ntrials per by phase (practice vs experimental trials)
        if phase == 'prac_cued' or phase == 'prac_free':
            ntrials = 16
        else:
            ntrials = 40
            
        for trial in np.arange(1,ntrials+1): # trial loop
            
            # set instructions based on phase
            ...
            # set correct index in the stimulus list
            ...
            keys = ['g', 'h','backspace'] if data.loc[i,'phase'] == 'cued' or data.loc[i,'phase'] == 'prac_cued' else ['s','d','k','l','backspace']           
...
end.draw(); win.flip(); event.waitKeys()
